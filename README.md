...
* **Raw MS data in mzML format**. Files can include MS1 and DDA-MS2 scans. DIA-MS is not supported. **All data must be centroided**. Centroided mzML can be obtained with ProteoWizard. Discard all profile data and always prioritize the centroid data provided by vendor's software, e.g. with the filter "peakPicking vendor msLevel=1-2". 
