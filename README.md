...
SLAW is a scalable containerized untargetted LC-MS processing workflow. The three strong points of SLAW are:
...
Some example data are given the test_data folder of this git folder. These data have been heavely filtered to allow a quick testing. An example of input folder is given in the _test_data/mzML_ folder and an example of parameters file (which will be generated by SLAW if you run it on an empty folder) is given in _test_data/parameters.txt_ an example of the complete output of SLAW without optimization is given in _test_data/output_. A zipped file containing the inputs, the only thing needed to run SLAW a  can be downloaded

Once you have donwloaded this folder and extracted at the location of your choice (PATH_INPUT), and create an empty anywhere to store the output (PATH_OUTPUT) the workflow can be run by opening a terminal, or on windows a Powershell (NOT Powershell ISE) and running
```
docker run --rm -v PATH_FOLDER\mzML:/input -v PATH_OUTPUT:/output adelabriere/slaw:latest
```
Normnally if you specified the path correctly and the output should be.
```
...
```
Now a _parameters.txt_ which stores all the parameters of the processing. You can then run the processing by running the command line again, SLAW take the input parameters.txt and process the data.
```
docker run --rm -v PATH_FOLDER\mzML:/input -v PATH_OUTPUT:/output adelabriere/slaw:latest
```
To avoid long processing time, the optimization is swtiched off by default. You can turn it on in the _parameters.txt_ file by setting the _optimization/need_optimization_ parameters to "True". If you choose to do so the processing should take less than 1 hour to finish. If not it should take less than 5 mins. If the workflow finished the
...
```
The output are generated in PATH_OUTPUT the completes output are:
 * datamatrices: The complete table with row corresponding to features or ions each column corresponding to a sample. Three flavours of datamatrices are possible.
 * fused_mgf: The consensus mfg spectra obtained, storing one ms-ms spectra by features in the data matrices.
 * OPENMS/CENTWAVE/ADAP: Store the individual peaktables and ms-ms spectra for each samples.

 More informations about the parameters and the workflow [can be found into the wiki.](https://github.com/adelabriere/SLAW/wiki).
