...
class PeakPickingOpenMS(PeakPicking):
    def __init__(self,row,min_fwhm,max_fwhm,snt,ppm,min_int,max_outlier,min_points,quant):
        self.input = row[3]
    ...
    def command_line_processing(self):
        return " ".join(["FeatureFinderMetabo","-in",self.input,"-out",self.output,
                         "-algorithm:common:chrom_peak_snr",str(self.snt),"-algorithm:common:noise_threshold_int",
                         str(self.min_int),"-algorithm:epd:masstrace_snr_filtering false -algorithm:epd:max_fwhm",
                         str(self.max_fwhm),"-algorithm:epd:masstrace_snr_filtering false -algorithm:epd:min_fwhm",
                         str(self.min_fwhm),"-algorithm:ffm:use_smoothed_intensities true -algorithm:mtd:mass_error_ppm",
                         ...
                         str(self.max_outlier),"-algorithm:mtd:min_trace_length",str(self.min_points)])
