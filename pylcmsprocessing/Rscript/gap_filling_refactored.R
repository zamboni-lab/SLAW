...
NUM_FILES <- as.integer(args[6])
NUM_WORKERS <- as.integer(args[7])
...
seq_samples[1] <- 0
if (seq_samples[length(seq_samples)] != num_features) {
  seq_samples <- c(seq_samples, num_features)
}
...
for(isample in 1:length(seq_samples)){
  batch_idx <- (seq_samples[isample]+1):seq_samples[isample+1]
  missing <- apply(dm[,batch_idx],2,function(x){!is.na(x)})
  #We insert them into the hdf5
  for(idx_sample in seq_along(missing)){
    feat_prefix <- paste(FEATURES_GROUP,"/",MISSING_FEATURES_PREFIX,batch_idx[idx_sample],sep="")
    h5write(missing[[idx_sample]],HDF5_FILE,feat_prefix)
  }
  #We get the potential isotopes
  max_isos <- apply(dm[,batch_idx],1,function(x){
    pmax <- which.max(x)
  ...
  found_isotopes <- which(max_isos[,2] != 0)
  if(length(found_isotopes)>0){
    max_isos_val <- max_isos[found_isotopes,1]
    max_isos_sample <- batch_idx[max_isos[found_isotopes,2]]
...
unique_samples <- unique(raw_isotopes_samples)
for(samp in unique_samples){
  sel_isos <- which(raw_isotopes_samples==samp)
  if(length(sel_isos)>0){
    iso_prefix <- paste(ISOTOPES_GROUP,"/",ISTOPES_TO_FIND_PREFIX,samp,sep="")
    h5write(sel_isos,HDF5_FILE,iso_prefix)
  }
}
...
extractMissingInformations <-
  function(praw,
           peaks,
           idx_isos,
           path_isotopes,#The hdf5 file path
           path_infer,#The hdf5 file path
           #isotopes,
           #infer,
           align,
           table_iso,
...
#This is the function which actually performs the gap-filling.
