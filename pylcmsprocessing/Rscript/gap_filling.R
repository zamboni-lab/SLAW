...
cnames <-fread(PATH_DM,sep=",",header = TRUE,nrows = 1)
...
for(idx in 1:(length(batches)-1)){
  ...
  dm <- fread(input = PATH_DM,nrows = batches[idx+1]-batches[idx]-1,skip=batches[idx])
  colnames(dm) <- cnames
  ...
  if(is.list(max_sample)){
    max_sample <- sapply(max_sample,function(x){x[1]})
  }
    ...
    if(length(isotopes)!=0){
      ...
      extractIsotopes <- function(peak,xraw,
        ...
        for(charge in max_charge:1){
          ...
          idx <- 1
          while(idx<=max_iso && !is.null(miso[[idx]])&& sc[miso[[idx]][1],2]<=max_theo_int[idx]){
            miso[[idx]] <- miso[[idx]][1]
            idx <- idx+1
          }
          if(idx==1) return(NA)
