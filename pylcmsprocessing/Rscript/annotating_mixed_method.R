  ...
  function(dm,
    ...
    for (i in 1:(length(seq_cut) - 1)) {
      ...
      if (!is.null(attr(ledges, "result"))) {
        ...
        ledges <-
          ledges[!sapply(ledges, function(x) {
            "bperror" %in% class(x)
          })]
      }
...
groupFeatures <-
  function(dm,
    ...
    if (cut_rts[cut_rts] != number_features) {
      cut_rts <- c(cut_rts, number_features + 1)
    } else{
      cut_rts[cut_rts] <- cut_rts[cut_rts] + 1
    }
